...
class ECP_File_Helper {
    ...
    public static function find_and_authorize_file( $file_key, $target_user_id ) {
        ...
        // For admins downloading on behalf of a user, we still need to check "All Users" files
        $search_all_users_fallback = $is_admin;

        $found_file = self::find_file_by_hash( $file_key, $user_context_id, $search_all_users_fallback );
        ...
        if ( ! $is_admin ) {
            ...
            // If it's not their own file and not an "All Users" file, deny access.
            if ( ! $is_own_file && ! $is_all_users_file ) {
                return new WP_Error( 'permission_denied', 'You do not have permission to download this file.' );
            }
            // If it is an "All Users" file, check if the current user has been excluded.
